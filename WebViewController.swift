...
//            self.statusbarView.backgroundColor = Constants.splashscreencolor
        ...
        #endif
        let _phonecheck = UIScreen.main.bounds
        let _statusbar: CGFloat = 0
        ...
        if Locale.current.languageCode?.lowercased() ?? "lang_unavailable" == alternatelanguage1_langcode?.lowercased() && 
            alternatelanguage1_langcode?.count ?? 0 > 1 {
        ...
        if (uselocalhtmlfolder)
        {
            ...
            webServer.start(withPort: 8000, bonjourName: "GCD Web Server")
            
            if let serverURL = webServer.serverURL {
                print("local_server: \(serverURL)")
                let phpFileURL = serverURL.appendingPathComponent("test.html")
                webView.load(URLRequest(url: phpFileURL))
            }
//            let urllocal = URL(fileURLWithPath: Bundle.main.path(forResource: "test", ofType: "html")!)
...
//            })();
//            """
//        webView.evaluateJavaScript(corsDisableScript) { (_, error) in
//            if let error = error {
...
// MARK: - WebViewcontroller : QRScanner
